...
- Create a `Kompanion` .NET core (net10.0) project and add it to the solution. This will contain the core functionalities of the framework, the first of which is integrating the `Get-OllamaProcesses`, `Invoke-ServeOllama` and `Invoke-StopOllama` of `kompanion.ps1` into a `OllamaService` class. This class will provide methods to get the list of running Ollama processes, serve a model, and stop a model.
...
- In a previous prompt a *giggling* feature was requested; it was in fact *jiggling* that was meant. Please implement mouse jiggling (and remove the giggling) every 10 seconds when KompanionUI is running to prevent the screen from locking, which is enabled by default. This will involve simulating small mouse movements at regular intervals. Ensure that this feature can be easily enabled or disabled from the "Settings" tab in the UI, allowing users to choose whether they want the mouse jiggling functionality active or not. Update the README.md to include instructions on how to enable or disable the mouse jiggling feature, and ensure that it is clearly explained for users who may not be familiar with this concept.

- It it would be useful to be able to inspect the status of the repositories without having to open VSCode. Add a "Status" button next to each repository in the "Repositories" tab, which when clicked, will display the output of `git status` for that repository in a popup window. This will allow users to quickly check the status of their repositories without having to launch VSCode. Ensure that the output is formatted in a user-friendly way, and that any errors encountered while executing the command are properly handled and displayed to the user. Update the README.md to include instructions on how to use the new "Status" button functionality, and ensure that it is clearly explained for users who may not be familiar with Git commands.

- Beside "Refresh", add a button that checks the status of all repositories (no pop-up with results) and updates the UI to indicate if there are any changes (e.g., uncommitted changes, ahead/behind status with the remote). Add a red indicator to show if there are any changes to the left of the repository name, and green if everything is up-to-date. Initialize this indicator in gray before the check is run for the first time. This will provide users with a quick overview of the state of their repositories without having to check each one individually.

- Add a checkbox under the settings for "Use Antigravity if available". The command for Antigravity is `antigravity.cmd`. If it exists and is runnable, use it instead of VS Code for launching the repositories. Ensure that the application checks for the existence of `antigravity.cmd` and its ability to run before using it as the default option for launching repositories. Update the README.md to include instructions on how to enable this feature.