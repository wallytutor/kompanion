...
from sympy import Eq as SymEq, expand, solve
...
Before starting we declare a few classes grouping the required symbols for what follows:
...
class BcVars:
    ...
    h = symbols("h")
    c_inf = symbols("c_{\\infty}")
```

```{python}
#| echo: true
class CellVars:
    """ Cell-centered coefficients and variables. """
    c_0 = symbols("c_0")
    c_E = symbols("c_E")
    c_P = symbols("c_P")
    c_W = symbols("c_W")
    ...
    c_G = symbols("c_G")
    c_B = symbols("c_B")  # sort of cell (0-D cell)...
...
#| echo: true
class FaceVars:
    """ Face-centered coefficients and variables. """
    D_e = symbols("D_e")
    D_w = symbols("D_w")
    D_g = symbols("D_g")
    D_b = symbols("D_b")
...
```{python}
#| echo: true
def diffusion_flux(D, c_left, c_right, delta=FvmVars.delta):
    """ Compute the (forward difference) diffusion flux between two points. """
    return -D * (c_right - c_left) / delta


def diffusion_flux_east(D=FaceVars.D_e, c_right=CellVars.c_E,
                        delta=FvmVars.delta):
    """ Compute the diffusion flux to the east cell. """
    return diffusion_flux(D, CellVars.c_P, c_right, delta=delta)


def diffusion_flux_west(D=FaceVars.D_w, c_left=CellVars.c_W,
                        delta=FvmVars.delta):
    """ Compute the diffusion flux to the west cell. """
    return diffusion_flux(D, c_left, CellVars.c_P, delta=delta)
```

For standardizing equations as organized as $F(x)=0$ we override `sympy.Eq` with `StandardEquation` class:

```{python}
#| echo: true
class StandardEquation(SymEq):
    """ Standard LHS format of finite volume equation. """
    def __new__(cls, lhs, rhs=0, **kwargs):
        return SymEq.__new__(cls, expand(lhs), 0, **kwargs)

    def __truediv__(self, other):
        return StandardEquation(self.lhs / other)

    def __rtruediv__(self, other):
        return StandardEquation(other / self.lhs)
    ...
    def subs(self, *args, **kwargs):
        ...
        return StandardEquation(self.lhs.subs(*args, **kwargs))

    def solve_for(self, var):
        """ Solve the equation for the specified variable. """
        return solve(self, var)
...
Below we compose the diffusion equation with help of [SymPy](https://docs.sympy.org/latest/index.html) and the above extensions. Notice that when compositing the right-hand side `rhs` we multiply by -1 because we are integrating the negative of the fluxes.
...
```{python}
#| echo: true
def fourier_number(D, tau=FvmVars.tau, delta=FvmVars.delta):
    """ Compute the Fourier number for the given parameters. """
    return D * tau / delta**2


SUBS_FOURIER = [
    (fourier_number(FaceVars.D_e), FaceVars.Fo_e),
    (fourier_number(FaceVars.D_w), FaceVars.Fo_w),
    (fourier_number(FaceVars.D_g), FaceVars.Fo_g),
    (fourier_number(FaceVars.D_b), FaceVars.Fo_b),
]
```
...
class StandardProblem:
    ...
    def get_rhs(**kw):
        ...
        J_e = kw.get("J_e", diffusion_flux_east())
        J_w = kw.get("J_w", diffusion_flux_west())
        return -1 * (J_e - J_w)
...
```{python}
#| echo: true
def convection_flux(cb, h=BcVars.h, c_inf=BcVars.c_inf):
...
Before simplifying this expression to find out $c_{B}$ it is worth introducing the [Sherwood number](Sherwood%20number%20-%20Wikipedia) $\mathcal{Sh}$ defined as follows, where $L$ represents a characteristic length of the problem and $D$ the diffusion coefficient; here $L=\delta_{P}/2$ as half-cell length was used in flux computations and
